...
fn partial_evaluate<F: PrimeField>(
    hypercube: &Vec<HypercubePoint<F>>,
    index: usize,
    eval_point: F,
) -> Vec<F> {
...
fn total_evaluate<F: PrimeField>(points: Vec<F>, evaluations: Vec<F>) -> Vec<F> {
    let dim = (points.len() as f64).log2().ceil() as usize;
    let mut hypercube = boolean_hypercube(points);

    for i in 0..dim {
        let new_points = partial_evaluate(&hypercube, dim - 1 - i, evaluations[dim - 1 - i]);
        hypercube = boolean_hypercube(new_points);
    }
...
mod tests {
    ...
    fn test_partial_evaluate() {
        ...
        let partial_eval = partial_evaluate(&hypercube, index, eval_point);
